...
- **The `origin/main` the locks compare against moves on any fetch, and a linked worktree may fetch.** `--sync` and `--pr` both fetch in the invoking checkout; refs/remotes are shared through the common dir, so a worktree running `cargo run -- --sync` updates the ref every checkout's sysroot check reads — it touches the primary's `main` only as a `--ff-only` on a clean primary. A `Wait` refusal is about another checkout's un-landed sysroot, never about this worktree's merge state.
- **A linked worktree's `main` ref is the primary checkout's branch and is only as current as its last `--sync`: anything asking "does this branch differ from main" must diff against `origin/main`** — a stale `main` attributes main's own commits to your branch.
...
- **Documentation carries no gates** — `src/docs.rs` and every test over spec prose were deleted by owner ruling (`8d0db10`), and the spec corpus itself followed on 2026-08-19; `src/redlist.rs` resolves doc paths only because it gates a Rust table, not a corpus.
