...
//! # The zone recovery
//!
//! [`resolve`] is the whole of the wall-clock question, and [`Recovery`] is its
//! honest answer type. The syscall surface gives userland two readings of one
//! instant — `SYS_CLOCK_EPOCH`, which is UTC seconds, and `SYS_CLOCK_REALTIME`,
//! which is local `h:m:s` and no date — so the offset between the zones is a
//! subtraction of seconds-of-day. That
