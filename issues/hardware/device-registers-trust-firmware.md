...
The userspace-driver and IOMMU tracks rest on the same dependency — that
firmware's MTRRs make the PCI hole uncacheable
(`issues/kernel/every-driver-is-still-in-the-kernel.md`,
`issues/kernel/the-iommu-stops-at-translation.md`); this is the entry that says
the machinery to remove it now exists.
