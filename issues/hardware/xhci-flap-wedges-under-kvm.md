...
kvm if it doesnt something with the guest is wrong" — and it is the class `issues/hardware/`'s
...
thing that changed. `issues/hardware/`'s own entries name the shape it would be in — one
...
`issues/build/parallel-tests-red-under-other-suites.md`'s classification
