...
- **A ticket lock cannot survive an abandoned waiter.** `sync.rs`'s
  ...
  "locks a dead thread can strand" class in `issues/panic-path/`, and worse than the held-guard
...
instruction, the shape `issues/kernel/` has been describing from T14 logs and that nothing has
...
lock `issues/boot-media/` calls out for exactly that. On the T14 there is no serial device so each
