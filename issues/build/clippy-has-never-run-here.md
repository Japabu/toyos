...
| **adopted so far** | **85** (`mm` 35 + `elf` 23 + `loader` 8 + `object` 1 + `sched` 8 + `iommu` 8 + `log` 2) | gated: each area's entry module (`mm/mod.rs`, `object/mod.rs`, `elf/mod.rs`, `loader/mod.rs`, `sched/mod.rs`, `iommu/mod.rs`, `log/mod.rs`) opens with `#![warn(clippy::undocumented_unsafe_blocks)]`, which composes with `host-tests.yml`'s existing `-D warnings` on both kernel invocations — no command-line change needed, since the kernel is one crate with no `-p` scoping and the module attribute is what stands in for it |
| **remaining** | **304** (`drivers` 121 + `arch` 107 + root files 76) | measured 2026-08-22 by re-running both kernel invocations with `-W clippy::undocumented_unsafe_blocks`: 304, which is the 322 this row carried minus the 18 the three small areas closed. Not gated — follow-up |
