...
| **adopted so far** | **188** (`mm` 35 + `elf` 23 + `loader` 8 + `object` 1 + `drivers` 121) | gated: each area's entry module (`mm/mod.rs`, `object/mod.rs`, `elf/mod.rs`, `loader/mod.rs`, `drivers/mod.rs`) opens with `#![warn(clippy::undocumented_unsafe_blocks)]`, which composes with `host-tests.yml`'s existing `-D warnings` on both kernel invocations — no command-line change needed, since the kernel is one crate with no `-p` scoping and the module attribute is what stands in for it |
| **remaining** | **201** (`arch` 107 + root files 76 + `sched` 8 + `iommu` 8 + `log` 2) | measured 2026-08-22 by re-running both kernel invocations with `-W clippy::undocumented_unsafe_blocks`: 201, which is the 322 this row carried minus the 121 `drivers/` closed. Not gated — follow-up |
