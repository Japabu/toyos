...
**The refusal rule says the opposite, and names exactly one exception, which is
not this one.** `kernel/src/object/handle.rs`'s `HandleError::refuse_as_error`
sends `BadHandle`, `Stale` and `WrongType` to `process::handle_fault`, which
ends the offending process; the sole carve-out is the connector argument to
`SYS_NAMESPACE_BUILD`. So the tree has a second exception that
...
oversight. Either it becomes the rule's second named exception or it becomes a
kill; both are rulings, and the answer belongs beside the refusal rule itself —
`kernel/src/object/handle.rs` — once it is made.

Found by the mechanism-consolidation audit of 2026-08-15, filed separately when
its three siblings were fixed.
