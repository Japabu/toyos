...
1. **Print `result.serial` in that arm**, and in every arm shaped like it — the
   ...
   diagnosis. `issues/build/a-failure-message-drops-the-lines-before-the-test-started.md`
   is the same hole seen from the other end.
2. Boot it with `BootOptions { qmp: true, .. }` so the guest survives the
   verdict and `human-monitor-command` with `info registers -a` over the lane's
   socket can say what the other CPU was doing. Take that capture before
   injecting anything — a keystroke revives a halted CPU, so Ctrl+Alt+D both
   confirms the diagnosis and destroys the evidence for it.
