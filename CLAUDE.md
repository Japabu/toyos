...
- **`memmap2` fork: mapping a file corrupts the caller's file cursor.** `MmapInner::read_file` in our `src/toyos.rs` does `file.try_clone()?` then seeks — but `try_clone` *shares* the file offset, so mapping silently leaves the caller's `File` seeked to the end of the mapped range, and any later read on that handle returns wrong data or EOF. Found 2026-07-28 by running upstream's own suite against our module (5/16 fail; this causes 3). The fix is portable and host-testable — save `stream_position()`, seek, read, seek back — and needs only `Seek`, which ToyOS std has; ToyOS std has no `FileExt`/`read_at`, so the positional-read fix is unavailable. This matters: memmap2 is `tier = "toolchain"`, backing `rustc_data_structures` in the self-hosted rustc.
- **`memmap2` fork: `map_mut` silently discards writes.** It reports success and then no-ops in `flush`, so mapped writes are lost. Upstream's contract since 0.9.10 is that unsupported platforms return `ErrorKind::Unsupported` precisely so callers can detect this. Either implement real write-back or return `Unsupported`; reporting success is the one option that is wrong. Mapped pointers are also only byte-aligned (backing `Vec<u8>`) where the crate guarantees `usize` alignment. See `forks.toml` for why this fork is deliberately not upstreamed.
