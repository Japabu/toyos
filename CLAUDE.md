...
- **FIRST-METAL-BOOT BLOCKER: the xHCI driver panics above three USB slots** (`device.rs:87-94`), and the T14 has four USB devices (camera, Bluetooth, fingerprint, boot stick). metal-sim has one, so it cannot reach it. Two more xHCI first-boot risks filed alongside it: no xECP/USBLEGSUP ownership handoff before the unconditional HCRST (QEMU cannot fail this; Lenovo firmware can), and USB hotplug does nothing at all — reachable now that M1 removed the zero-HID panic.
...
- **`sys_read` blocks on an empty Keyboard fd but returns `NotFound` on an empty Mouse fd.** Two fds of the same shape, two answers.
...
- **Build/fork debt:** `bootstrap-cc` is alive but not wired in (nothing builds it, wrong toolchain, unpinned TinyCC download); the `memmap2` fork is 165 lines of unreachable code — delete `src/toyos.rs` or drop the toyos gate in `rustc_data_structures`, exactly one of the two.
