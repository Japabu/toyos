...
**Storage** — `block::BlockDevice` is the one device interface, in whole 4 KiB blocks, and **every method is fallible**: a failed read must not be indistinguishable from data. NVMe and USB mass storage implement it. The USB side is xHCI bulk endpoints plus Bulk-Only Transport with a transparent SCSI command set (`drivers/xhci/msc.rs`), which is how a machine that boots off a stick can read the stick. A device's own numbers — block size, capacity, CSW tag, residue, endpoint address — are untrusted: checked, and the device refused by name, never truncated to fit. **An optional SCSI command's refusal is an answer, not a device failure**: `msc_flush` reads SYNCHRONIZE CACHE's ILLEGAL REQUEST / INVALID COMMAND OPERATION CODE as "this stick has no write cache", because reading it as failure made the ESP log's idle-loop flush a permanent write loop on the stick the machine booted from — 45 flushes in 89 ms, measured. Corollary with reach beyond USB: **an error path that logs is an error path that produces work for the thing that failed**, so `FileSystem::sync` is fallible and `esp_log` disables its sink on the first real failure rather than logging one.

**Input** — One held-set and one button-merge for the whole machine (`keyboard::handle_key`, `mouse::handle_motion`), so two keyboards or two pointers compose rather than contradict — across two xHCI controllers as well, which is what a `PointerSource` numbered at bind rather than derived from a per-controller slot id buys. USB HID over xHCI and PS/2 over the i8042 both feed it in HID usage codes; wire decoding is `toyos-ps2/`. Pin interrupts arrive through `drivers/ioapic.rs`; everything else is MSI-X, or MSI (`PciDevice::enable_msi`) where a function has no MSI-X table — and an xHC with neither is refused by name, never driven blind. **The FADT's `IAPC_BOOT_ARCH` 8042 bit does not gate the i8042 probe and must not be made to again** — the T14 clears it on a machine whose keyboard is PS/2, and the driver's own handshake is better evidence than a vendor's summary bit; a floating bus (`0xff` from port 0x64) is what makes a machine with no controller refuse in one `inb`.
...
- The harness has more machine shapes than the CLI: `Profile::MetalUsb` is metal-sim with six devices on the xHCI (two keyboards), the config `xhci_many_devices` and `xhci_slot_exhaustion` run on — QEMU cannot starve a controller's slot count, so the latter uses the `xhci-one-slot` kernel feature as its actuator. Five more features stage what a device or a controller does that QEMU's cannot: `usb-flush-unimplemented`/`usb-flush-fails` (a stick that will not flush versus one that cannot), `xhci-deaf-controller`/`xhci-deaf-port` (a bring-up register bit that never settles — before the deadlines those two boots hung), and `xhci-descriptor-selftest` (nine crafted configuration descriptors through the pure parser). Four more put a *second* USB disk beside the boot stick: `UsbDisk`, `UsbDisk4k` (4 KiB sectors), `UsbDiskHuge` (3 TB, more sectors than READ(10) addresses, so the driver must refuse it) and `UsbDiskReadOnly` (a write-protected LUN — the only place in the suite where a real device refuses an I/O the driver was right to issue). Ground truth is the backing file on the host; the guest half is the `usb-storage-gate` kernel feature, and **the stamp in block 0, not the flag, decides which disk it may write** — the boot stick is on the same bus.
...
- **A red test may be the build system, not the code — re-run in isolation before believing any single red.** `'rustc' is not installed for the custom toolchain 'toyos'`, or a `stage1-std/<target>/dist/deps` temp-dir error, means a concurrent build and never a broken checkout; the tell is that the same command succeeds between attempts. Wait and re-run — **never repair or force-rebuild the toolchain**, which makes it worse for everyone. Three such windows have been found here; two are closed, the std-bootstrap one is not (known-issues §6).
...
- **`esp_log`'s flush is unbounded and uninterruptible, in `idle_loop` before `pass()`.** Anything added to the idle loop is an audio change, and userland `println!` shares that ring.
