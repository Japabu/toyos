...
- **soundd's TPDF dither does not linearize the quantizer — spec §5.4 is not met.** `dither_and_quantize` is `(sample * 32767.0 + dither) as i16`, and Rust's `as i16` truncates *toward zero*; proper TPDF requires rounding. Measured transfer function (400k trials/point, input in LSB → mean output): `+0.25 → +0.03`, `+0.50 → +0.13`, `+1.00 → +0.50`, `+2.50 → +2.00`, `−0.50 → −0.13`; with rounding each returns the input exactly. So there is a 2-LSB dead zone at zero (crossover distortion), a systematic 0.5-LSB magnitude compression at every level, and a noise floor that *is* modulated by signal level — exactly what §5.4 says TPDF must prevent. **Fixing it also breaks the audio test harness:** `tests/common/audio.rs::zero_runs` detects dropouts by `s == 0` exactly, which only works because truncation maps a silent mix to digital zero. With rounding, silence becomes ±1 dither noise and the gap detector goes permanently, silently green. The quantizer and the detector's zero test must change in the same commit (detector needs an amplitude floor, not an equality test).
