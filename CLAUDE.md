...
- **No slop comments.** Never add comments that restate what the code does. No "auto-closes on drop", "returns the value", "loop through items". Comments explain *why*, not *what*. If the code needs a *what* comment to be understood, rewrite the code. If a comment reads like a commit message — evidence, measurements, what the code used to do — it belongs in the commit message. The form that slips through is a *why* beside an assertion that already enforces it: the assertion is the statement, compiler-checked, so the comment is restatement with a weaker guarantee. The tell is a comment that locates code — "the assertion below" — which addresses the reader of a diff, not the file, and rots when the code moves.
...
**Filesystem** — VFS with mount points. Initrd, tmpfs, NVMe, USB. **The kernel never formats a disk it was not given, and a failed mount is not consent** — a disk it cannot positively identify as ours is never written to. The ESP is FAT32 by UEFI mandate rather than by legacy, and is treated as untrusted input like any other disk. **A mount's extent is the adapter's invariant, not the filesystem's to be trusted about**: clamp at the kernel boundary whatever the driver promises. A mount is named for its role, never its format — naming it after its format is one step from selecting it by its format. **An operation that fails must not have destroyed what it was asked to replace, nor kept what it took**: write and insert first, free last, and give back every run a failed allocation loop reserved — `bcachefs`'s create, delete, rename and metadata paths all had this backwards. **A backing is revoked, not kept alive, when its file goes**: the blocks are the next file's the moment the entry is gone, so a stale read must fail rather than serve them, and every backing for a name reads through one shared `FileBlocks`.
...
- **The suite runs several guests at once, and a test is serial until it says otherwise.** Every boot owns its image, socket and scratch files; every worker owns a lane of the pid's temp dir (`tests/common/lane.rs`). Each `MACHINE_TESTS`/`SCREEN_TESTS` entry carries a `Sched` and does not compile without one — parallel phase, then a serial tail of everything with a wall-clock margin on either clock (and the shared block, whose per-test ceilings are one), then **gate A alone**, which asserts the live-guest count is zero rather than trusting its position. `cargo test -- --jobs N` sets the width. `group_of`'s adjacent runs are one task on one worker, and a shared boot's console is drained *by* its members. The harness builds at opt-level 2, and a boot's kernel, bootloader and initrd are memoized per run on what each is a function of — so a source edit mid-run is not picked up, because a run is a measurement of one tree. Costs and what each bought: `specs/test-cost-audit.md` §5.1–§5.3.
