...
- **The whole of a syscall is a preempt-disabled section**, so `preempt::enable`'s slow path is not the RT-wake safe point scheduler spec §7.4 counts on, and wake latency in syscall context is bounded by the syscall, not by a lock section. Deterministic since the preempt count became conserved across switches; before that it was true at random. Also: `retire_task` is never reached by `cargo test`.
