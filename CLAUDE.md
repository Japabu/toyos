...
- **Every guest binary is built with `[profile.toyos]`** — opt-level 2, debug info, debug-assertions and overflow-checks **on**. There is no `--release` flag: it bundles four knobs and turns two of them off silently, and both crafted-ELF kernel panics were *found* by an overflow check. `build::assert_overflow_checked` refuses to write an image whose kernel does not reference `attempt to add with overflow`, and `assert_actuators_match_features` one whose kernel names an actuator. `userland/libc` is the one exception and says so where it is.
- **A full `cargo test` builds two kernels — the one every image ships and one carrying every actuator, armed by name at boot** (`kernel/src/actuator.rs`, `--kernel-param`). An actuator is never a cargo feature: a third kernel build costs a rebuild of the kernel per run forever, so `cargo test --lib` makes you declare one, and `fpu-save-nothing` is the only one that earned it. **Whatever measures boots the shipping kernel**; `usb-slow-device`, `io-depth-probe` and `control-regs-bench` are the exceptions and their numbers are the test kernel's.
- Boot modes, the boot parameter, the build lock and the host's guest/build slots: `src/CLAUDE.md`. The harness, machine shapes, gate A and expected failures: `tests/CLAUDE.md`.
