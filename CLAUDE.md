...
- **A red test may be the build system, not the code — re-run in isolation before believing any single red.** `'rustc' is not installed for the custom toolchain 'toyos'`, or a `stage1-std/<target>/dist/deps` temp-dir error, means a concurrent build and never a broken checkout; the tell is that the same command succeeds between attempts. Wait and re-run — **never repair or force-rebuild the toolchain**, which makes it worse for everyone. Three such windows have been found here and all three are closed: the build system's stateful phases (toolchain bootstrap, sysroot writes, the `cargo clean`s) now decide *and* act under one exclusive section of a repo-level lock — `src/buildlock.rs`, files in `.build-locks/` because a clean removes every other candidate location — while ordinary builds hold it shared and still run concurrently. A `[build-lock] waiting …` line naming a holder is that working, not a hang. Only `./x.py` typed by hand in `rust/` escapes it.
