...
- **Neither cache evicts.** The block page cache never removes a slot, and `file_cache::init` has zero call sites so its `max_pages` stays `usize::MAX` and `evict_if_needed` is dead. Both grow for the life of the boot.
