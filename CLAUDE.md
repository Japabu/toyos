...
**Filesystem** — VFS with mount points. Initrd, tmpfs, NVMe, USB. **The kernel never formats a disk it was not given, and a failed mount is not consent** — a disk it cannot positively identify as ours is never written to. The ESP is FAT32 by UEFI mandate rather than by legacy, and is treated as untrusted input like any other disk. **A mount's extent is the adapter's invariant, not the filesystem's to be trusted about**: clamp at the kernel boundary whatever the driver promises. A mount is named for its role, never its format — naming it after its format is one step from selecting it by its format.
