...
- `specs/scheduler-core-spec.md` — ownership-typed scheduler core as a `no_std` crate
  ...
  **Migration state: Stage 7a done — the kernel now drives the core.**
  ...
  **Off at 7a:** `Env::steal` is `false`, so `StealRequest` and balance are
  disabled and placement is spawn-time (rotating least-published-load) plus
  wake-time local. `retire_task` is still synchronous — it posts `Msg::Retire`
  and then yields until the state word reads `Dead`, because process teardown
  frees memory the target's page tables still map. Both are Stage 7b.
  **Scope note:** 7a deleted what the cutover orphaned, because dead code is a
  ...
  address-space-freed-under-a-live-task detector).
...
- **An involuntary pass between `prepare_wait` and `block_on` panics the kernel.** Found 2026-07-29 by splitting the simulator's block into two steps and then offering `Step::Pass` in the gap: it aborts at once on `toyos-sched/src/invariants.rs:53` — `running task disagrees with its state word: left Committing(CpuId(1), Gen(2)), right Running(CpuId(1))`. The kernel reaches the same window: `WaitQueue::prepare_wait` (`toyos-sched/src/waitq.rs:133-134`) publishes `Committing` and *then* takes the leaf lock; `KernelLock` is a `Lock` (`kernel/src/sched/payload.rs:38-42`) whose guard `Drop` calls `preempt::enable()` (`kernel/src/sync.rs:84-89`), which at count 0 with `need_resched` set runs `scheduler::do_preempt()` → `driver::pass(Dispose::None)` → `SchedPass::preempt_if_due` → `RunningTask::preempt`, asserting `Running(cpu) → Ready(cpu)` on a `Committing` word. The window is not the few instructions inside `prepare_wait` either — `scheduler::wait_until`'s `ready()` re-check runs there too, and every re-check that takes a lock ends in the same `enable()`. A ring-0 timer fire sets `need_resched` without preempting (`kernel/src/arch/idt/timer.rs:96`) and means the quantum has expired, so `preempt_if_due` fires. Not fixed: the obvious guard (skip preemption on a `Committing` task) leaves `quantum_end` in the past and re-arms the timer to it, so this is a protocol decision, not a one-liner. Until it is settled, `Vm::enabled` deliberately does not offer a pass mid-block and says so.
- **A `Retire` that lands while its target is between `prepare_wait` and the park is dropped, and the thread is never reaped.** Found 2026-07-29 the same way. `CpuSched::handle_retire` (`toyos-sched/src/cpu.rs:521-527`) answers a *running* target with `need_resched` and consumes the message, on spec §7.6's promise that it "dies at its next safe point". Parking is not a safe point: `pass_block` drains, drops the retire, then `dispose_block` parks the thread — and a parked task is never picked, so nothing reaps it. It sits `Blocked` with the kill bit set, holding its process's address-space reference, which is the crash.md family. Reproduced in ~33 of 400 sim seeds on `crash_md_exit_race` by deleting the `kill_pending` arm of `Vm::block_pass`; the sim keeps that arm because it models the per-op safe point, and the comment there points here. Candidate fixes both need discussion: re-post the retire from `dispose_block` (the node is free — the consumer released it — but a fresh retire posted between the drain and the park would make the claim fail, and `post_retire` `expect`s it), or refuse to park a killed task (which livelocks a thread whose blocking site loops).
...
- **7a is gated red on smp=8 only, and it is expected rather than a defect to tune.** The thorough tier after the `underruns` fix (`f25fa87`) fails on a pooled ceiling-breach rate: 10 of 120 config-runs vs a recorded 0 of 120, Fisher p=8.03e-4. Every one of the ten is an **smp=8** config — `drains` 5>4 and 10/7/8>6, `wake_lat` 52-63 ms against a 48 ms limit and 105 ms against 80 ms. **Zero breaches at smp=1**, where 7a is a large improvement (wake lateness 13.9 ms median → ~7.6 ms, `wakes` 426-496 → ~1010). The split is the diagnosis: 7a ships with `StealRequest`/balance **disabled** by design (spec §11, "wake-time push placement only"), so a task woken onto a busy CPU cannot migrate and simply waits — which is a multi-CPU tail and nothing at all on one CPU. **7b enables StealRequest and balance**, and is the sub-stage that should close it. Do not re-baseline and do not tune the scheduler to this; re-gate after 7b. If 7b does *not* close it, that is a real regression in the wake path and the ceilings were right.
