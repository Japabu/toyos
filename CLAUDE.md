...
- **The host is one host, and worktrees change nothing about it.** Concurrent measurement stays unreliable: another agent's build or QEMU boot perturbs timings whichever tree it runs in, the 14 cores are one budget spent once, and gate A's baseline was recorded on a quiet host. A/B in the same session against the same HEAD, never against a number recorded earlier. `specs/test-cost-audit.md` §4.1 prices this and its constraint 4 is the owner's open decision.
