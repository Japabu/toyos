...
- **Two xHCI first-boot risks remain**: no xECP/USBLEGSUP ownership handoff before the unconditional HCRST (QEMU cannot fail this; Lenovo firmware can), and USB hotplug does nothing at all — reachable now that M1 removed the zero-HID panic. Also: a slot is never given back (non-HID devices keep theirs), and OP_PAGESIZE is logged but not honoured.
