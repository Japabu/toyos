...
**Input** — One held-set and one button-merge for the whole machine, so two keyboards or two pointers compose rather than contradict. Sources are numbered where they bind, never by an index a device or controller supplies, and given back when the device is unplugged — `mouse::unbind` is the only thing that frees one and it clears the buttons first. USB HID and PS/2 both feed it in HID usage codes; wire decoding is `toyos-ps2/`, and layouts, dead-key composition and `locale detect`'s question table are `toyos-keymap/` — host-tested, `us` still the default, and the pending diacritic is one per machine for the same reason the held-set is. **A byte a decoder is still holding is not a byte that decoded to nothing** — conflating them made two thirds of a healthy pointer stream read as a desync and cost a field investigation. The i8042's counter line repeats *only* when the pin has asserted since the last one, so in a log its silence is evidence that no interrupt arrived, not that the driver stopped looking.
...
toyos-keymap/     Keyboard layouts, dead-key composition, layout detection
