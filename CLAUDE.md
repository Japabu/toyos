...
- **The working tree, the index, and the branch are all shared state — agents reason about the first, are surprised by the second, and do not think about the third at all.** **Treat every commit as a race.** Never `git add -A`/`git add .`; stage explicit paths, check `git diff --cached --name-only`, and prefer `git commit -- <paths>`, which commits only those paths and leaves anyone else's staged work alone (applies inside the forks too). **Put the `add` and the `commit` in one tool call**: narrow hunks shrink the blast radius but the window between them is real, and another agent's bare `git commit` has taken a staged hunk from `tests/toyos.rs` twice. **When a change adds a new file and edits a shared file that references it, commit the new file first, on its own** — callee before caller, for commits as well as edits; a sweep that takes the referencing half and leaves the defining half behind gives you a HEAD that compiles neither. **A swept hunk is fixed forward, never undone** — undoing is history rewriting, which is forbidden anyway and is strictly worse than a wrong commit message. **And an untracked file in the shared tree is already in everyone's build** — `build_toyos_bins` enumerates `src/bin/*.rs`, so a throwaway reproducer is in every other agent's `cargo test` from the moment it is written, not from when it is committed. One that existed for three minutes and was correctly deleted still cost another agent a diagnosis. Put throwaways somewhere `build_toyos_bins` does not scan. **Never create or switch git branches.** Six agents share one checkout, so there is one HEAD between all of them: `git checkout -b` moves *everyone* onto the new branch and their commits land there instead of `main`, silently. This happened today — an agent branched to commit a fix, every other agent's commits went onto that branch while it existed, and the fix was not in `main`. Repaired by fast-forwarding `main` and switching back, safe only because the trees were identical. Commit to `main`, always.
...
- **Line atomicity on serial: FIXED at `8de0a95`, and it was libc, not the kernel.** `FILE` had no buffer, so one `printf` became several `write` syscalls and the splice happened between them. The known-issues heading had blamed the virtio-console for months. Checking the premise cost one command over logs already on disk — 151,047 lines, 37 splices, **zero cutting a kernel line** — and moved the fix to a different crate. `tests/common/audio.rs`'s reader-side workaround can go once this is confirmed in a full run.
...
- **Two scheduler leaks/looseness:** a thread retired while parked leaks its wait-queue node (the intrusive `wait_node` the core still owes fixes it); `handle_retire`'s `need_resched` on a running target is a request the next pass may decline (conformant — §7.6 bounds it by the quantum). **The fair split degrades as the machine widens, and it is the policy — settled, both halves measured**: worst service spread 30 ms at 1 CPU to 1386 ms at 32, an offset that saturates rather than drifting, in the shipped decide-who-runs code. Mechanism: every running thread of a process charges one pot, so the pot advances at the process's aggregate rate while each queued thread's key stays frozen — one dispatch of staleness buys more service the more of that process runs at once. Same root as §9.2's tie-break: per-process accounting with per-thread queueing. Magnitude is worst-of-N over adversarial interleavings, not average behaviour. **Separately, it crosses its own derived granularity bound at 4/6/8/12 CPUs**; the gate reds on `max(derived, allowance)` but records every derived-bound crossing, so the allowance cannot become the standard. `src/build.rs` cannot enable `sched-check`, so no CI run exercises the check build.
