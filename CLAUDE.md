...
- **Gate A's fast tier fails on two counters after the Stage 7a cutover, reproducibly, with the zero-gap histogram clean.** Measured over 6 `cargo test` invocations (24 config-runs) on a quiet host. The reproducible one is `audio_tone_load` smp=1: `underruns` 98–121 against a ceiling of 70 and a recorded 30-run sample of 2–33, every single run. `audio_tone` smp=8 fails `max_wake_lat_us` on about half its runs (54–65 ms against a 48 ms limit) and passes the rest — bimodal, not drifted. Everything else is in range, and the *harm* measure is better than the baseline: 24 config-runs produced 2 gap events, against a recorded 3-of-30-suites dropout rate. What actually changed is soundd's service rate: on `audio_tone_load` smp=1 its `wakes` went from a recorded 426–496 to 1001–1049 — i.e. the CPU hog no longer starves it, which is the RT-preemption fix (B7) doing its job — and its mix cycles now arrive *before* the client has refilled, which is what `underruns` counts. Three hypotheses were tested and refuted: the run-queue tie-break (fixed, real, changed nothing here), a shorter RT boost window (made it worse: underruns 137 and an 8-period gap), and removing the client boost entirely (underruns unchanged at 102, but two real gaps appeared — so the boost is load-bearing for audibility and is not what moves this counter). `underruns` also tracks `drains` at almost exactly 8× (the batch size), so the two are one signal, not two. **This needs the thorough tier to settle, and possibly a re-baseline: the ceilings were calibrated against a scheduler that starved soundd.** Do not tune the scheduler to chase these counters without deciding that question first.
