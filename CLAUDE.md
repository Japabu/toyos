...
- **Nothing under `/tmp` is loadable** (`vfs.rs:62` has no `open_backing`), and `/home` panics above ~2 MiB, so userland cannot create a loadable file larger than about 2 MiB at all — which is why the ELF allocation-ceiling tests assert on declared length rather than reaching the heap assert. VA exhaustion is likewise untestable (the PMM refuses first).
