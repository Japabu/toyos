...
- **A crafted ELF still panics the kernel.** `vaddr_to_file_offset` (`loader.rs:265`) has no failure path. Separately, `SYS_DLOPEN` never dedups and `SYS_DLCLOSE` is a no-op, so VA exhaustion reaches `.expect` at `syscall.rs:1435`. More of the same class, from the sweep after the T14's first boot: `align_2m` wraps on unchecked adds in `elf.rs:994` and `loader.rs:63`; `SYS_SYSINFO` collects one entry per live thread; the bootloader allocates whatever the ESP reports as a file size; the NVMe driver trusts the namespace's sector size into a shift and a divide.
