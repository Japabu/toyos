...
- **A full `cargo test` builds two kernels: the one every image ships and one carrying every actuator, armed by `--kernel-param`** (`kernel/src/actuator.rs`). An actuator is never a cargo feature, a third is refused, and **whatever measures boots the shipping one**.
