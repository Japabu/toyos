...
[Typhoon Peak]", listed **undriven** in `specs/metal-hardware-inventory.md:126`
...
**Userland, under netd, IOMMU-confined**, per `specs/userspace-drivers-spec.md`.
...
from `specs/audio-gate-history.md` and it applies here with full force.
...
with `/log/kernel.log`. The pipeline exists (`specs/metal-log-capture.md`,
...
`specs/net-gate-plan.md` tests the stack — TCP echo, lifecycle, adversarial
...
`specs/metal-hardware-inventory.md` names the I219-V Ethernet at `00:1f.6` as
