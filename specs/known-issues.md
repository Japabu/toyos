...
### OPEN, UNASSIGNED — every AP runs with caching disabled, `CR0.NE` clear, `CR0.WP` clear and `CR4.MCE` clear
...
**`NE` clear explains §1's unexplained survivor.** `fault_gates`' `mf` arm
...
### OPEN, UNASSIGNED — the audio pops are four spinlocks, not one spinning driver, and the suite can now stage them
...
The two entries below are the fixes that came before. Each was right about its
own defect; all of them chose which CPU absorbs a stall whose duration none of
them touched.
