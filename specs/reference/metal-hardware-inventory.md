...
- **SMI generation: `USBLEGCTLSTS 0xe0000000 -> 0x00000000`.** The five SMI enable
  ...
  `issues/hardware/`, that the port-0x60/0x64 SMM trap is disarmed before
...
- **The FADT denies its own 8042 and is wrong.** `iapc_boot_arch=0x0011` is
  ...
  in `issues/hardware/`; it is repeated here because it is a *property of this
...
- **The byte decodes.** Both boots report `1 bytes, 1 keys`. The open item in
  `issues/hardware/` — "one byte reached the kernel and produced no event" — is
...
(`issues/hardware/`, measured on the `metal_sim_compositor` boot) and `(234ms)` for
...
`issues/hardware/hotplug-blocks-a-scheduler-pass.md` — has never been
