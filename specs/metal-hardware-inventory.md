...
Neither log states the machine's total RAM. `specs/metal-boot-plan.md` records
...
`specs/metal-boot-plan.md` M5 plans a native I2C-HID touchpad as "LPSS I2C driver,
...
- **The EC is slow but inside budget.** From the controller self-test to the armed
  ...
  budget of 2100 ms total (`specs/metal-boot-plan.md` M2: 250 + 500 + 750 + 600).
...
2. **`specs/metal-boot-plan.md`: "No USB mass-storage driver is needed".** True of
...
3. **`specs/metal-boot-plan.md` M2 quotes `armed at 1460ms` and `1 bytes, 0
...
5. **`specs/metal-boot-plan.md` M4: "the T14's four internal devices enumerate".**
