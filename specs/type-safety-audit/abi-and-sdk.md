...
known-issues' "a client's request is an allocation request", with a thread as
...
The kernel has not panicked here since the hardening pass known-issues records at
...
This is known-issues §1's class exactly, in a userland daemon's namespace, and
...
Distinct from known-issues' "the bootloader sizes every allocation from a file
...
- **§5.2's deferred zero-handle queue.** It is the largest piece of new
  ...
  (known-issues §2). The machinery earns its place.
