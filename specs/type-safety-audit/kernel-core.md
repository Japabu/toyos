...
on an unknown type. Not hypothetical for this tree: known-issues §1 records that
...
matters once grant has a revoke, which known-issues §1 keeps open deliberately —
...
**Relation to filed work.** Extends known-issues §7's `KernelSlice::from_raw`
...
- **The parameter is named `lba` and carries a 4 KiB filesystem block index.**
  ...
  (known-issues §3). Passing a device LBA where a block index is expected is an
...
> failure mode `specs/known-issues.md`'s §1 postscript records.
...
**A correction while here.** CLAUDE.md and known-issues §1 both say
...
- `PipeId`, `ListenerId`, `RingId`, `SharedToken` as designations (known-issues
...
- `FileBacking` outliving unlink and `NvmeBacking::read_page` re-deriving a block
  from stale extents (known-issues §1, deliberately unassigned pending the
...
- `gpu::set_resolution` freeing the framebuffer under its consumers
  (known-issues §7). The `INFO` overwrite also drops the old `GpuInfo`'s three
  `SharedToken`s with no `unregister` — that is the `SharedToken`-has-no-RAII
  item, not a second one.
- `RingHeader`'s `u32` counters wrapping at 4 GiB (known-issues §1, ASSIGNED).
...
- The io_uring watcher triple (`add_io_uring_watcher` / `remove_io_uring_watcher`
  ...
  `wake_waiters` nothing links it to. This is known-issues §3's "An io_uring
...
- `SYS_LISTEN` having no namespace, so `listener::listen(name: &str, ..)` is
  stringly-typed by design for now (known-issues §1). `Descriptor::Listener` is
...
- `sys_read` blocking on Keyboard and returning `NotFound` on Mouse
  (known-issues §3), with `waitqs::MOUSE`/`NETWORK` having wakes and no waiters.
...
## What deserves promotion to `specs/known-issues.md`
