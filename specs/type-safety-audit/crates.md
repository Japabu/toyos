...
`specs/known-issues.md` §1 still lists "A 3 MiB `fs::write` to `/home` panics the
...
is parsed. `specs/known-issues.md` §3 already asks the owner to rename the crate;
...
situation exactly (`specs/metal-boot-plan.md`), and `specs/known-issues.md` §3
...
`specs/known-issues.md` §2's open "the VFS lock is the same shape" class.
...
This is `specs/known-issues.md` §1's stated lesson verbatim — *a policy enforced at
...
**This is `specs/known-issues.md` §1's filed class in a new shape.** That entry
...
This is the shape `specs/known-issues.md` §1 prefers to a bound — *count by type,
...
`specs/known-issues.md` §1's class header ("an id or a name treated as a
...
- **`Mounted::list` unbounded** — already filed (`specs/known-issues.md` §1). Not
...
`apply_timer` (`cpu.rs:905-906`) — and `specs/known-issues.md` §3 records that
...
turns this into `specs/known-issues.md` §2's stranded-lock class, and the one `if`
...
`specs/known-issues.md` §3 files the leak and correctly names the owed intrusive
...
**Sequencing note, not an objection.** `specs/known-issues.md` §3 sets three entry
...
- **`WaitList`'s unbounded `VecDeque`** — one `Arc` clone per waiter, bounded by
  live thread count, which is itself uncapped (`specs/known-issues.md` §1,
...
- **The per-process fairness degradation and its three entry criteria** — filed in
  full (`specs/known-issues.md` §3). Untouched here; S6 says so explicitly.
...
- **The `0xAA` reset ambiguity** — `specs/known-issues.md` §3 files it. A property of
...
binary, which is worse than a crash: `specs/known-issues.md` §1's standing
...
| B2 | bcachefs | `root_level` from disk drives unbounded recursion | yes — kernel stack overflow | shared with B1 | yes — known-issues §1 |
| B3 | bcachefs | partial allocation read as complete; write lands outside its extent | **yes — live data corruption on `/home`** | −1 `unreachable!`, 3 callers converge | yes — known-issues §1, ignored failure return |
| B4 | bcachefs | `with_capacity` from a `u16` on disk, 387× the physical max | yes — kernel panic | +3 | yes — known-issues §1 |
| B5 | bcachefs | superblock fields never validated against the device | yes — arbitrary metadata writes | 24 `BlockNum::new` split two ways | yes — known-issues §1 |
...
| S4 | toyos-sched | `!Sync` re-derived by a runtime flag at the boundary | no — correct today | 0 | adjacent to known-issues §2 |
| S5 | toyos-sched | the wait-registration drop bomb cannot fire on the kill path | **yes — the filed leak, plus O(n) dequeue** | −`VecDeque`, −`AtomicBool`+3 accessors, O(n)→O(1) | yes — known-issues §3 files the leak, not the cause |
...
8. **S2, S1, S6, S3** — S6 wants the `thread_coverage_pct` A/B that
   `specs/known-issues.md` §3 criterion 2 demands of anything near `pop_surplus`.
