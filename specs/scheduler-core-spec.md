...
kernel/src/sched/                # replaces kernel/src/scheduler.rs at the cutover stage
  mod.rs                         # kernel-facing API; preserves the syscall surface; entry asserts
...
`kernel/src/sched/mod.rs` entry shims assert, before constructing any pass:
...
- **Stage-5 shim fidelity**: the transitional `ticket.fired()` recheck must be reviewed per
  source; the shim is deleted at 7c, so any residual subtlety is temporary by construction.
