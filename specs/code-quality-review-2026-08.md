...
- **`i8042/`** — **OPEN.** Not a rewrite (its shape is two field investigations
  ...
  verdict recorded**; see the hardware-name entry in known-issues §7.
- **`panic_console/`** — **AFFIRMED, no rewrite, and the owner's own question
  ...
  (known-issues §2, "Nothing distinguishes `panic_console::capture` from a
...
§1.5. **OPEN**, and now filed with its measurements as known-issues §7 rather
...
- **`process.rs`** — **OPEN**, 1,743 lines. Target: the lifecycle
  ...
  *owns*, the state machine what it *is*. #142/#156 (known-issues §3) are the
...
- **Log subsystem** — **OPEN, and the owner's note was a question, not a
  ...
  question in known-issues §7 with the file inventory attached.
- **Kernel layout** — **OPEN, question for the owner.** 39 flat `.rs` files in
  ...
  Same known-issues §7 entry as the log question, because the owner asked both
  in one note.
...
- **`mm/region.rs`** — **OPEN, already filed.** The file's own TODOs are the
  ...
  type whose purpose is bounds. Covered in full by known-issues §7,
...
- **`toyos/` SDK** — **OPEN.** Full API-surface audit: ownership-correct handle
  ...
  crates.io and upstream reviewers. Overlaps known-issues §7's "`Fd` is a
...
11. **OPEN** — the comment-policy sweep + hardware-name scrub + gates
    ...
    measurements in known-issues §7.
12. **OPEN** — small deletions from §2 (gdt shim, empty AP hook, debug tools,
    percpu items) — can ride any adjacent task. Filed in known-issues §7.

## 4. The review channel

`jan-review` was a message channel, not code: notes were read as a diff against
main, converted to verdicts here and entries in known-issues, and never landed.
...
|---|---|---|---|
| 1 | `bootloader/main.rs:164` | why so long comments? | §2 comment policy; known-issues §7 comment sweep |
...
| 5 | `arch/mod.rs` | `#[allow(dead_code)]` on `debug` — why is that needed? | §2 arch/ — OPEN, five uncalled tools; known-issues §6 + §7 |
...
| 8 | `arch/percpu.rs` asserts | verifies against constants but doesnt guarantee the constants are the same used in e.g. `preempt.rs` | **known-issues §7** — 47 `gs:[N]` sites bound to nothing |
...
| 11 | `bcachefs_adapter.rs:17` | "now runs the whole way" thats narration slop | known-issues §7 comment sweep — still present |
...
| 14 | `drivers/i8042/mod.rs` | never mention ThinkPad in the kernel source code | **known-issues §7** — 6 ThinkPad + 59 T14 sites in kernel `.rs` |
...
| 21 | `log.rs` | should we redesign and rewrite the log subsystem and rethink the kernel's file/folder structure? | **known-issues §7 — OPEN QUESTION for the owner**, evidence attached |
| 22 | `log_file.rs` | theres slop narration in the comments | known-issues §7 comment sweep — 46% comment lines |
| 23 | `main.rs:3` | `#![allow(dead_code)]` why? | **FILED `451edff`** — known-issues §6, 49 hidden warnings |
...
| 25 | `main.rs` (end) | the whole codebase has too many comments. good code speaks for itself | **known-issues §7** — the consolidated entry |
| 26 | `mm/region.rs:12` | look at all TODOs | **already filed** — known-issues §7 `KernelSlice::from_raw`; still exactly two, tree-wide |
| 27 | `process.rs` | huge. crate? test? | §2, deep dive 7 — OPEN, 1,743 lines |
| 28 | `sched/driver.rs` | so many comments … refer to the spec and let the code speak? | known-issues §7 comment sweep |
...
| 31 | `user_ptr.rs:246` | `#[allow(dead_code)]` why? audit all places of this | **ANSWERED `4353289`** — `user_slice_of_mut` deleted; the audit is known-issues §6 |
