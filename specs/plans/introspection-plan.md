...
`specs/plans/daemon-testability.md` proposes extracting `mix_pass` behind a
...
print one line and exit. `specs/plans/net-gate-plan.md` wants per-daemon counters for
...
`specs/assessments/metal-log-capture.md` asks how to get bytes off a machine with no 16550,
...
- **It is not authorization.** Any process can call `SYS_QUERY` and get a
  ...
  *mistake* and against *TOCTOU*; it does not defend against a hostile process,
  ...
  `specs/assessments/capability-handles-spec.md`'s question.
...
> unaffected; only the numbers are wrong.
