...
The kernel's log ring sits one line behind on an idle machine (`specs/issues/diagnostics/`), so a guest
...
which costs three boots. The fix is `specs/issues/diagnostics/`'s — a drain that does not need the machine
to be busy.
