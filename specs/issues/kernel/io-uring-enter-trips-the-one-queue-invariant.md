...
**It is not the path
`specs/issues/kernel/keyboard-flood-panics-blocked-read.md` records.** That one
...
## 2026-08-16: the path in the capture no longer exists, and the invariant is
narrower than it was

`specs/completion-architecture-spec.md` C3 took every blocking site off the
...
of that thread, and the capture does not establish which wait that was. After
C3 the search is much smaller — the only queue a task can be on is its own, and
