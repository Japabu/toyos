...
itself is recovered too. Gated in both directions under `serial_vocabulary`, and
against the real `#DF` `double_fault_stack` already puts on the wire.
...
- **the LAPIC's spurious-interrupt vector is `0xFF` and the IDT has no entry at
  ...
  `specs/issues/kernel/the-lapic-spurious-vector-has-no-gate.md`. **It is not
...
`specs/issues/kernel/ring0-jump-to-zero-under-port-polls.md` and
`specs/issues/kernel/a-ring-0-fetch-at-0x1b-during-a-loaded-boot.md`) to say so.
