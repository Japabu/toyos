...
`specs/issues/hardware/xhci-flap-wedges-under-kvm.md` records it twice: red alone
...
counted before it caught up". `specs/issues/hardware/eleven-names-red-on-ci.md`
carries the same 0 of 5 as one of the twelve that came off the list, and
`specs/issues/build/parallel-tests-red-under-other-suites.md` has a third,
...
- **244 s in the phase on a machine with one guest on it.** CI is `--jobs 1`,
  one guest per runner (`specs/ci-plan.md` §8), so the dev host's contention
