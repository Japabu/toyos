...
**The signature's cause is closed: `specs/issues/kernel/`, two CPUs shooting down at once.** It was a
...
on `specs/issues/kernel/`'s two backtraces and on `an_initiator_answers_while_it_waits`, which is
red without it.
...
Every one of those reds is the parallel-red list in `specs/issues/design-debt/`, not this entry: the two
...
squarely `specs/issues/design-debt/`**, whose own last paragraph says a verdict that flips with the host
...
`69` on two CPUs of a two-CPU guest — which is the same pair of initiators `specs/issues/kernel/`'s
backtraces name.
