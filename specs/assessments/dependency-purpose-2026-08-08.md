...
The companion to `specs/assessments/dependency-audit-2026-08-08.md`. That document asked
...
- **Called at** `userland/doom/build.rs:...` — `http_agent()` builds an
  ...
  `agent.get(...).call()`.
- **Why** one HTTPS GET of
  `github.com/ozkl/doomgeneric/archive/<DOOMGENERIC_COMMIT>.tar.gz`. It was
  `refs/heads/master.tar.gz` when this was written; the pin landed the same day.
...
- **Called at** `build.rs` — `tar::Archive::new(gz)`, then the entry loop that
  strips `doomgeneric-<commit>/doomgeneric/` and unpacks the rest.

### 8.7 Should the download exist once it is pinned?

The audit's §6 asked for the download to be pinned, and it now is —
`DOOMGENERIC_COMMIT` is `fc60163`, `forks.toml` `[doomgeneric]` records it under
a `source` tier, and `doomgeneric/.toyos-commit` makes the pin bind a checkout
that already has a tree. The question left over is whether pinning is enough.
Measured, not argued:
