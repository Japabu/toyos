...
| 16 | The xHCI slot is leaked "on four paths" | `device.rs:225-231`, `issues/` | **Undercounted** — 11 exits (F12) |
| 17 | Per-device EP0 rings removed one of hotplug's three obstacles | `issues/` | **Holds**, and there are more obstacles left than the record names (F13) |
...
grows to `MAX_LOG_BYTES` and rotates forever, and `issues/`' "the ESP log's
...
`issues/panic-path/panic-console-capture-untested.md` — a recovery path whose only evidence of having run
...
**Location.** `issues/hardware/xhci-slot-never-given-back.md`;
`device.rs:225-231`.
...
Also: `issues/` lists "a disk whose interface has no bulk pair" as one of
...
memcpy. `issues/` records the ESP flush at 2.0–9.7 ms per flush against a
...
on 500M spins. `issues/` already files "one command at a time per
...
- **Multiple LUNs, UAS, CBI/CB, READ(16)/WRITE(16), removable media, MODE
  ...
  in `issues/filesystem/usb-mass-storage-not-implemented.md`, each with its
