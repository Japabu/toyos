...
`issues/` was read in full first. Where a finding extends a filed
...
invariant in prose; `issues/hardware/` records that hotplug is precisely what
...
1. **The copies have already drifted, in the direction that matters.** A machine
   ...
   `issues/audio/soundd-panics-on-a-shallow-pipeline.md`).
...
- **Two of the numbers being believed are real bugs, and neither is the table
  ...
  `issues/isolation/` rather than fixed, because the fix changes that function's
...
filed `FileBacking`-after-unlink leak (`issues/isolation/`, *"reads blocks the
...
"filed rather than changed here" but no entry exists in `issues/`.
...
4. **A legal firmware shape kills the boot.** `init_power:203`
   ...
   panic filed in `issues/isolation/` and the xHCI zero-HID panic M1 closed.
...
reachable from the dev host, and `issues/` has no ACPI entry at
...
**Standing. New — no ACPI entry exists in `issues/`.**
...
(`issues/design-debt/`). That entry names `from_raw` and three constructors. **It does
...
hand, and it is the function whose stage-summing arithmetic `issues/hardware/`
...
stops repainting after `Boot: complete` (`issues/hardware/`).
...
`issues/panic-path/` records for the *panic* path only.
...
- **`exceptions.rs:317-321` writes raw to port `0x3F8` with no `uart_present()`
  ...
  machine with no SuperIO reads `0xFF` from that port, and `issues/panic-path/`
...
(`issues/design-debt/`) says why: the caller must survive the window even if the callee
refuses, which a `Drop` cannot express. Making it a guard would be worse.

**`SharedToken` as a bare `u32`.** Filed (`issues/design-debt/`). Not re-filed.
...
`issues/isolation/`'s postscript says to: *"In a tree with six agents committing, the
...
findings would duplicate `issues/isolation/`). Also read for context: `mm/mod.rs`,
`mm/mmio.rs`, `mm/region.rs`, `block.rs`, `page_cache.rs` and `usb_gate.rs` (read
paths), CLAUDE.md, `issues/` in full, `specs/assessments/metal-track-history.md`
in full.
