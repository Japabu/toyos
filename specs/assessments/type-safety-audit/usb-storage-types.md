...
This extends `specs/assessments/type-safety-audit/usb-storage.md` (F1–F13) and
...
triaging (`issues/`, the cache-eviction investigation: *"grep over the
...
invariant that the `issues/` entry asserts and that is currently 2/3 true.
...
- **`esp_log`'s "It costs nothing when nothing is logged".** Filed — CLAUDE.md's
  `issues/` entry carries the correction (userland `println!` is in the same
