...
- **A wake is not proof of its condition** — a task is woken *by name* as well as by queue (`thread_exit` wakes its process's main thread whatever that thread is waiting for; a futex bucket is shared), so a park that returned proves only that the thread runs again. `scheduler::wait_until` re-checks and re-parks; a site that reaches for `prepare_wait`/`block_on` directly owns that loop itself, and `sys_nanosleep` is the one that does not — `specs/issues/kernel/nanosleep-ends-early-when-a-sibling-thread-exits.md`.
