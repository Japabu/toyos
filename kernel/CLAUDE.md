...
- **No disk wait in this kernel can park, and the driver is not why** — at the moment a transfer is waited for, the CPU is four ticket spinlocks deep, each disabling preemption for its whole life. `issues/kernel/every-wait-in-this-kernel-is-a-spin.md` is the track; `issues/audio/disk-wait-pins-a-cpu.md` the entry.
