...
- **`BackendGuard` masks interrupts for its whole life, so anything written under it is an interrupt latency** — the console drain is bounded to eight records for that reason, and a new holder must bound itself too.
