...
pub fn find_device(ecam_base: u64, class: u8, subclass: u8, prog_if: Option<u8>) -> Option<(u8, u8, u8)> {
    for bus in 0..=255u16 {
        for dev in 0..32u8 {
            let vendor_id = ecam_read_u16(ecam_base, bus as u8, dev, 0, 0x00);
            if vendor_id == 0xFFFF { continue; }

            if matches_device(ecam_base, bus as u8, dev, 0, class, subclass, prog_if) {
            ...
            if header_type & 0x80 != 0 {
                for func in 1..=7u8 {
                    ...
                    if matches_device(ecam_base, bus as u8, dev, func, class, subclass, prog_if) {
...
fn matches_device(ecam_base: u64, bus: u8, dev: u8, func: u8, class: u8, subclass: u8, prog_if: Option<u8>) -> bool {
    let c = ecam_read_u8(ecam_base, bus, dev, func, 0x0B);
    let sc = ecam_read_u8(ecam_base, bus, dev, func, 0x0A);
    if c != class || sc != subclass { return false; }
    match prog_if {
        Some(pi) => ecam_read_u8(ecam_base, bus, dev, func, 0x09) == pi,
        None => true,
    }
}
