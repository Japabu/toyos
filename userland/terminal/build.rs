...
fn main() {
    ...
    // Codepoints to include: Latin + Box Drawing + Block Elements
    ...
    // Find the largest px_size where printable ASCII fits in the cell
    ...
    loop {
        ...
        assert!(
            px_size > 4.0,
            "Could not find a font size that fits in {}x{}",
            FONT_WIDTH,
            FONT_HEIGHT
        );
    }
    eprintln!(
        "font: selected px_size={:.2} for {}x{} cell",
        px_size, FONT_WIDTH, FONT_HEIGHT
    );
    ...
    // Font format: [u32 count][u32 codepoints...][glyph data...]
    ...
    let font_path = Path::new(&out_dir).join("font.bin");
    std::fs::write(&font_path, &output).expect("Failed to write font bitmap");
    eprintln!(
        "terminal: generated font.bin ({} glyphs, {} bytes) from JetBrains Mono",
        glyph_count,
        output.len()
    );
