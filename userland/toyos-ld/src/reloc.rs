...
pub(crate) fn apply_relocs(
    ...
    if params.record_relatives {
        ...
        // Map GOT symbol → originating section (for local symbol resolution)
        let mut got_origin: HashMap<&str, SectionIdx> = HashMap::new();
        for r in &relocs {
            if matches!(r.r_type, RelocType::X86Gotpcrel | RelocType::X86Gotpcrelx
                | RelocType::X86RexGotpcrelx | RelocType::X86Gottpoff)
            {
                got_origin.entry(&r.symbol_name).or_insert(r.section);
            }
        }

        for (sym_name, &got_vaddr) in params.got {
            let from_sec = *got_origin.get(sym_name.as_str())
                .expect("GOT symbol has no originating relocation");
            let sym_addr = resolve_symbol(state, sym_name, from_sec, params.plt)
