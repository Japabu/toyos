...
**Every `cargo test` runs the *fast* tier and names what it did not run.** The owner's line, 2026-08-11: ten seconds or less per test. `src/tiers.rs`'s `RELEGATED` is the other 31, with what each cost and what it guarded; `-- --nightly` runs them. Each entry carries a `Tier` beside its `Sched` or does not compile, `check_registration` ties the two lists, and a group shares one tier because it shares one boot. Interim, pending #188: `specs/test-cost-audit.md` §7.
