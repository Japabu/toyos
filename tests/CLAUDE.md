...
- **Host suites** run with plain `cargo test` inside `toyos-sched/`, `toyos-ps2/`, `toyos-gpt/`, `toyos-elf/`, `toyos-cc/`, `toyos-ld/`, `toyos-hda/`, `toyos-pci/`, `toyos-xhci/`, `toyos-desktop/`, `kernel-loom/`, `kernel-span/`, `toyos-fat32/`, `toyos-fat32-check/`, `toyos-abi/`, `toyos-manifest/`; `userland/sshd` and `userland/calc` cross-compile and need the host triple (`cargo test --target "$(rustc -vV | sed -n 's/^host: //p')"`) — `calc`'s arithmetic is all decided there, so its 64 tests are the whole of what that program is gated by. `kernel-loom` is the only memory-ordering check in the tree — x86 TSO hides a missing acquire edge from every guest test.
